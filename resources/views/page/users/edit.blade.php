    ...
    <div class="container">
        ...
        <form action="{{route('users.update',$user->id)}}" method="post">
            ...
            <div class="form-group" style="margin-bottom: 10px">
                ...
                <input name="email" type="email" class="form-control" value="{{old('email',$user->email)}}" id="email"
            ...
            <div class="form-group" style="margin-bottom: 10px">
                ...
                <input name="birthday" type="date" class="form-control"
                       value="{{old('birthday',date_format (new DateTime($user->birthday), 'Y-m-d'))}}" id="birthday"
                       placeholder="">
                @error('birthday')
                <p class="text-danger">{{ $message }}</p>
                @enderror
            ...
            <div class="form-group" style="margin-bottom: 10px">
                ...
                <input name="tel" type="text" class="form-control" value="{{old('tel',$user->tel)}}" id="tel"
            ...
            <div class="form-group" style="margin-bottom: 10px">
                ...
                <select name="roleId" id="roleId" class="form-select">
                    @foreach($roles as $role)
                        <option
                            {{ $user->roleId == $role->id ? 'selected' : '' }} value="{{ $role->id }}">{{$role->type}}</option>
                    @endforeach
                </select>
                @error('roleId')
                <p class="text-danger">{{ $message }}</p>
