    ...
    <div class="container">
        ...
        <form action="{{route('report.update', $report->id)}}" method="post">
            ...
            <div class="form-group" style="margin-bottom: 10px">
                ...
                <select name="projectId" id="projectId" class="form-select">
                    @foreach($projects as $project)
                        <option
                            {{ $report->projectId == $project->id ? 'selected' : '' }} value="{{ $project->id }}">{{$project->name}}</option>
                    @endforeach
                </select>
                @error('projectId')
            ...
            <div class="form-group" style="margin-bottom: 10px">
                ...
                <select name="positionId" id="positionId" class="form-select">
                    @foreach($positions as $position)
                        <option
                            {{ $report->positionId == $position->id ? 'selected' : '' }} value="{{ $position->id }}">{{$position->type}}</option>
                    @endforeach
                </select>
                @error('positionId')
                <p class="text-danger">{{ $message }}</p>
            ...
            <div class="form-group" style="margin-bottom: 10px">
                ...
                <input name="workingTime" type="text" class="form-control"
                       value="{{old('workingTime',$report->workingTime)}}" id="workingTime" placeholder="Enter time">
                @error('workingTime')
                <p class="text-danger">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="form-group" style="margin-bottom: 10px">
                ...
                <input name="date" type="date" class="form-control" value="{{old('date'), $report->date}}" id="date"
